...
class TestInitCommand:
    def test_init_creates_whence_dir(self, tmp_git_repo):
        result = main(["init"])
        assert result == 0
        whence_dir = tmp_git_repo / ".git" / "whence"
        assert whence_dir.exists()
        assert (whence_dir / "config.json").exists()
        assert (whence_dir / "queue.ndjson").exists()

    def test_init_config_defaults(self, tmp_git_repo):
        main(["init"])
        config = json.loads((tmp_git_repo / ".git" / "whence" / "config.json").read_text())
    ...
    def test_init_already_initialized(self, whence_initialized):
        result = main(["init"])
        assert result == 2  # Already initialized


class TestRecordCommand:
    def test_record_simple(self, whence_initialized):
        result = main(["record", "--prompt", "Test prompt", "--tool", "claude-code"])
        assert result == 0

        # Check queue has the event
        queue_path = whence_initialized / ".git" / "whence" / "queue.ndjson"
    ...
    def test_record_no_response(self, whence_initialized):
    ...
    def test_record_with_context(self, whence_initialized):
...
class TestQueueCommand:
    def test_queue_count_empty(self, whence_initialized):
        result = main(["queue", "count"])
        assert result == 0

    def test_queue_count_after_record(self, whence_initialized):
        main(["record", "--prompt", "Event 1"])
        main(["record", "--prompt", "Event 2"])
        result = main(["queue", "count"])
        assert result == 0

    def test_queue_clear_requires_force(self, whence_initialized):
        main(["record", "--prompt", "Event 1"])
        result = main(["queue", "clear"])
        assert result == 1  # Requires --force

    def test_queue_clear_with_force(self, whence_initialized):
        main(["record", "--prompt", "Event 1"])
...
class TestAttachCommand:
    def test_attach_empty_queue(self, whence_initialized):
        result = main(["attach"])
        assert result == 1  # No events

    def test_attach_basic(self, whence_initialized):
        # Record an event
        ...
        sha = subprocess.run(
            ["git", "rev-parse", "HEAD"],
            capture_output=True, text=True, cwd=whence_initialized,
        ).stdout.strip()
        note = subprocess.run(
            ["git", "notes", "--ref=refs/notes/whence", "show", sha],
            capture_output=True, text=True, cwd=whence_initialized,
        )
        assert note.returncode == 0
        assert "WHENCE-Spec-Version: 0.1.0" in note.stdout
        assert "claude-code" in note.stdout

    def test_attach_dry_run(self, whence_initialized):
    ...
    def test_attach_hash_all_mode(self, whence_initialized):
...
class TestShowCommand:
    def test_show_no_trace(self, whence_initialized):
        result = main(["show"])
        assert result == 1  # No trace

    def test_show_after_attach(self, whence_initialized):
        main(["record", "--prompt", "Build the widget", "--tool", "claude-code"])
        main(["attach"])
        result = main(["show"])
        assert result == 0

    def test_show_json_format(self, whence_initialized):
        main(["record", "--prompt", "Build the widget", "--tool", "claude-code"])
        main(["attach"])
        result = main(["show", "--format", "json"])
        assert result == 0

    def test_show_verify(self, whence_initialized):
        main(["record", "--prompt", "Build the widget", "--tool", "claude-code"])
        main(["attach"])
...
class TestVerifyCommand:
    def test_verify_integrity_pass(self, whence_initialized):
        # Create a second commit so we have a valid parent for the range
        (whence_initialized / "test.txt").write_text("test\n")
        subprocess.run(["git", "add", "test.txt"], cwd=whence_initialized, capture_output=True, check=True)
        subprocess.run(["git", "commit", "-m", "Add test file"], cwd=whence_initialized, capture_output=True, check=True)
...
class TestFullPipeline:
    def test_end_to_end(self, whence_initialized):
        """Full pipeline: record -> attach -> show -> verify."""
        # Create a second commit so we have a valid parent for the range
        (whence_initialized / "feature.py").write_text("# new feature\n")
        subprocess.run(["git", "add", "feature.py"], cwd=whence_initialized, capture_output=True, check=True)
        subprocess.run(["git", "commit", "-m", "Add feature"], cwd=whence_initialized, capture_output=True, check=True)
